...
// registro item-id -> { w, h, pal:[hex], data:[índice de paleta por celda, 0=transparente],
//                       fw, fd (huella en casillas), solid, sit, anim, name, category }
const PLZ_CUSTOM = {};
...
// huella (en casillas) de un mueble: los custom pueden ocupar varias; los base ocupan 1×1
function plzFurnFoot(t) { const cu = PLZ_CUSTOM[t]; return cu ? { fw: cu.fw || 1, fd: cu.fd || 1 } : { fw: 1, fd: 1 }; }
// lista de casillas [i,j] que ocupa un mueble colocado (ancla = esquina posterior-izquierda)
function plzFurnTiles(f) {
  const { fw, fd } = plzFurnFoot(f.t); const t = [];
...
function plzCollisions(furn) {
  const blocked = new Set(), seats = new Map(), portals = new Map();
  for (const f of furn) {
    if (f.t === 'portal') { portals.set(f.i + ',' + f.j, f); continue; }
    const solid = plzIsSolid(f.t), seat = plzIsSeat(f);
    if (!solid && !seat) continue;
    for (const [ti, tj] of plzFurnTiles(f)) {
      if (solid) blocked.add(ti + ',' + tj);
      if (seat) seats.set(ti + ',' + tj, f);
    }
  }
  return { blocked, seats, portals };
}
// recomputa colisiones/asientos/portales tras editar el mobiliario en caliente
function plzRecompute() {
  if (!plzR) return;
  Object.assign(plzR, plzCollisions(plzR.furn));
}
...
async function renderPlaza() {
  ...
  const sendEmote = (kind, emoji, target) => {
    const payload = { id: state.user.id, kind, emoji: emoji || null, target: target || null };
    if (kind === 'react') {
      const t = target ? plaza.ents.get(target) : me;
      if (t) t.float = { emoji, until: performance.now() + 2600 };
    } else {
  ...
  const openMenu = (ent, cssX, cssY) => {
    ...
    menuEl.querySelectorAll('button').forEach(b => b.onclick = (ev) => {
      ...
      const a = b.dataset.a;
      if (a === 'profile') openProfile(ent.id);
      else if (a === 'dm') openDM(ent.id);
      else if (a === 'dance') sendEmote('dance');
      else if (a === 'wave') sendEmote('wave', null, self ? null : ent.id);
      else if (a === 'fist') sendEmote('fist', null, ent.id);
  ...
  // ---- tocar el lienzo: avatar → menú; suelo → caminar ----
  canvas.addEventListener('pointerdown', (e) => {
    ...
    const hit = [...plaza.ents.values()].sort((a, b) => (b.x + b.y) - (a.x + a.y)).find(en => {
      ...
      return rx >= bx2 - 11 && rx <= bx2 + 11 && ry >= base2 - 46 && ry <= base2 + 3;
    });
    if (hit) { openMenu(hit, (e.clientX - r.left), (e.clientY - r.top)); haptic(6); return; }
  ...
  const applyRadio = async (row) => {
    try {
      if (!row || !row.track_id || !row.started_at) { updNp(null); return; }
      const { data: t } = await sb.from('tracks').select('*, profiles!tracks_user_id_fkey(*)').eq('id', row.track_id).maybeSingle();
      if (!t || !plaza) return;
      let byName = '';
      if (row.set_by) { try { const { data: p2 } = await sb.from('profiles').select('username,display_name').eq('id', row.set_by).maybeSingle(); byName = p2 ? (p2.display_name || p2.username) : ''; } catch (_) {} }
      updNp(t, byName);
      const offset = Math.max(0, (Date.now() - new Date(row.started_at).getTime()) / 1000);
      if (!state.current || state.current.id !== t.id) {
        playTrack(t);
        const seek = () => { try { const d = audio.duration || t.duration || 0; audio.currentTime = d > 2 ? (offset % d) : 0; } catch (_) {} };
...
function plzAllEditItems() {
  // solo los objetos custom de verdad (id 'c_…'); los base editados ya están en PLZ_EDIT_ITEMS
  const custom = Object.keys(PLZ_CUSTOM).filter(t => t[0] === 'c' && t[1] === '_')
    .map(t => ({ t, n: PLZ_CUSTOM[t].name || t, c: PLZ_CATS.includes(PLZ_CUSTOM[t].category) ? PLZ_CUSTOM[t].category : 'Deco' }));
...
async function plzOpenRoom(rec) {
  ...
  if (rec.owner_id === state.user.id || plzIsAdmin()) plzShowEditFab();
}
...
function openPlazaRoomsBrowser() {
  ...
  const load = async () => {
    ...
    data.forEach(r => {
      ...
      const mine = r.owner_id === state.user.id || plzIsAdmin();
...
const PLZ_BASE_SEATS = new Set(['bench', 'stool', 'sofa', 'couch']);
let plzRasterMode = false;   // al rasterizar un mueble base, no dibujamos su sombra de contacto

// convierte un mueble base (dibujado por código) en un sprite de píxeles editable.
// Renderiza el mueble a 1:1, recorta a su contenido y cuantiza los colores.
function plzRasterizeToSprite(type) {
  const S = 96, cv = document.createElement('canvas'); cv.width = S; cv.height = S;
  const c = cv.getContext('2d'); c.imageSmoothingEnabled = false;
  const ox = S / 2, oy = S - 24;   // apoya el mueble cerca del borde inferior
  const sp = plaza, sr = plzR;
  ...
  const w = bw * SPR_BLK, h = bh * SPR_BLK;
  const dx = Math.floor((w - cw) / 2), dy = h - ch;   // centrar en x, apoyar abajo
  const q = (v) => Math.min(255, Math.round(v / 17) * 17);   // cuantiza para limitar la paleta
  ...
  return { w, h, pal, data, fw: 1, fd: 1, solid: PLZ_SOLID.has(type), sit: PLZ_BASE_SEATS.has(type), anim: 'none' };
}
...
function plzFurnThumb(type, px) {
  ...
  const c = cv.getContext('2d'); c.imageSmoothingEnabled = false;
  const cu = PLZ_CUSTOM[type];
  if (cu) {
    // objeto custom: escala para que quepa entero (ancho o alto), centrado
    const pad = 6, s = Math.max(1, Math.min((px - pad * 2) / Math.max(1, cu.w), (px - pad * 2) / Math.max(1, cu.h)));
    c.setTransform(s, 0, 0, s, px / 2, px - pad);
    try { plzDrawCustomSprite(c, cu, 0, 0, 900); } catch (_) {}
    return cv;
  }
...
async function openPlazaShop(onBuy) {
  ...
  // Para objetos base (dibujados por código) los rasteriza a píxeles para poder editarlos.
  ...
  const render = () => {
    ...
    list.forEach(it => {
      ...
      const canEdit = admin || mine;                 // admin edita cualquiera; editor los suyos
      const canDelete = it.custom && (admin || mine); // los objetos base no se borran
      const card = el(`<div class="shop-card ${qty > 0 ? 'owned' : ''}">
        ${qty > 0 ? `<span class="shop-qty">×${qty}</span>` : ''}
        ${(canEdit || canDelete) ? `<div class="pobj-tools">${canEdit ? `<button class="pobj-edit" title="Editar objeto" aria-label="Editar"><svg fill="none" stroke="currentColor"><use href="#i-brush"/></svg></button>` : ''}${canDelete ? `<button class="pobj-del" title="Borrar objeto" aria-label="Borrar"><svg fill="none" stroke="currentColor"><use href="#i-trash"/></svg></button>` : ''}</div>` : ''}
      ...
      const editBtn = card.querySelector('.pobj-edit');
...
function openPlazaSpriteEditor(onDone, existing) {
  ...
  if (ed0 && ed0.sprite) {
    ...
    srcFrames = (Array.isArray(s.frames) && s.frames.length) ? s.frames : [Array.isArray(s.data) ? s.data : []];
    srcFps = Math.max(1, Math.min(20, s.fps || 6));
  }
  let w = bw * SPR_BLK, h = bh * SPR_BLK;
  // convierte un fotograma (índices de paleta) a celdas de color al tamaño actual (centrado x, apoyado abajo)
  const mapFrameData = (data) => {
    const oc = new Array(srcW * srcH).fill(null);
    for (let k = 0; k < srcW * srcH; k++) { const v = (data[k] | 0); if (v) oc[k] = srcPal[v - 1] || null; }
    const nc = new Array(w * h).fill(null), dx = Math.floor((w - srcW) / 2), dy = h - srcH;
  ...
  const m = openModal(`<div class="modal-head"><h3><svg class="mh-ic" fill="none" stroke="currentColor"><use href="#i-palette"/></svg> ${ed0 ? 'Editar objeto' : 'Crear objeto'}</h3><button class="close">&times;</button></div>
    <div class="modal-body spr2-body">
      <div class="spr2">
        ...
        <div class="spr2-center">
          <div class="spr2-canvas" id="sprView"><canvas id="sprCv" class="spr-canvas"></canvas></div>
          <div class="spr2-frames">
            <div class="spr2-framestrip" id="sprFrames"></div>
            <div class="spr2-framectl">
              <button class="spr2-fbtn" id="sprAddFrame" title="Añadir fotograma"><svg fill="none" stroke="currentColor"><use href="#i-plus"/></svg></button>
              <button class="spr2-fbtn" id="sprDupFrame" title="Duplicar fotograma"><svg fill="none" stroke="currentColor"><use href="#i-copy"/></svg></button>
              <button class="spr2-fbtn" id="sprDelFrame" title="Borrar fotograma"><svg fill="none" stroke="currentColor"><use href="#i-trash"/></svg></button>
              <button class="spr2-fbtn" id="sprOnion" title="Papel cebolla (ver fotograma anterior)"><svg fill="none" stroke="currentColor"><use href="#i-onion"/></svg></button>
              <button class="spr2-fbtn spr2-play" id="sprPlay" title="Reproducir/parar"><svg fill="none" stroke="currentColor"><use href="#i-play"/></svg></button>
              <label class="spr2-fps">FPS <input id="sprFps" type="number" min="1" max="20" value="${fps}"></label>
            </div>
          </div>
        </div>
        <div class="spr2-side">
  ...
  function drawGrid(fcOverride) {
    const cur = fcOverride || cells;
    gctx.clearRect(0, 0, gcv.width, gcv.height);
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) { gctx.fillStyle = (x + y) % 2 ? '#1a2036' : '#141a2c'; gctx.fillRect(x * cs, y * cs, cs, cs); }
    // papel cebolla: fotograma anterior en fantasma (solo al editar, no al reproducir)
  ...
  const renderFrames = () => {
    const host = m.querySelector('#sprFrames'); if (!host) return;
    host.innerHTML = '';
    frames.forEach((fc, idx) => {
      const b = el(`<button class="spr2-frame ${idx === curFrame ? 'on' : ''}" title="Fotograma ${idx + 1}"><canvas width="${w}" height="${h}"></canvas><span class="spr2-frame-n">${idx + 1}</span></button>`);
      const fx = b.querySelector('canvas').getContext('2d');
      for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) { const col = fc[y * w + x]; if (col) { fx.fillStyle = col; fx.fillRect(x, y, 1, 1); } }
      b.onclick = () => selectFrame(idx);
      host.appendChild(b);
    });
  ...
  const setPlay = (on) => {
    ...
    if (!on || frames.length <= 1) { drawGrid(); return; }
    const t0 = performance.now();
    const loop = (t) => { if (!playing || !document.body.contains(m)) { playing = false; drawGrid(); return; } const fi = Math.floor((t - t0) / (1000 / fps)) % frames.length; drawGrid(frames[fi]); playRaf = requestAnimationFrame(loop); };
  ...
  function resize(nbw, nbh) {
    ...
    const dx = Math.floor((w - ow) / 2), dy = h - oh;
    frames = frames.map(oc => { const nc = new Array(w * h).fill(null); for (let y = 0; y < oh; y++) for (let x = 0; x < ow; x++) { const v = oc[y * ow + x]; if (!v) continue; const nx = x + dx, ny = y + dy; if (nx >= 0 && nx < w && ny >= 0 && ny < h) nc[ny * w + nx] = v; } return nc; });
    cells = frames[curFrame]; undo.length = 0; redo.length = 0; updoBtns(); fitZoom(); sizeCanvas(); renderSteps(); renderFoot(); renderFrames(); redraw();
  }
  const renderSteps = () => {
  ...
  m.querySelector('#sprDelFrame').onclick = () => delFrame();
  m.querySelector('#sprOnion').onclick = () => { onion = !onion; m.querySelector('#sprOnion').classList.toggle('on', onion); drawGrid(); };
  m.querySelector('#sprPlay').onclick = () => setPlay(!playing);
  m.querySelector('#sprFps').onchange = (e) => { fps = Math.max(1, Math.min(20, parseInt(e.target.value, 10) || 6)); e.target.value = fps; };
  ...
  setCur(curColor); renderRecent(); renderSteps(); renderFoot(); updoBtns(); renderFrames();
  const refit = () => { fitZoom(); sizeCanvas(); if (!playing) drawGrid(); zoomPct(); };
...
function openPerkyInvaders() {
  ...
  const wrap = el(`
    <div class="pk-game" id="pkGame">
      ...
      <div class="pk-stage pixel" id="pkStage"><canvas id="pkCanvas" width="${AW}" height="${AH}"></canvas></div>
      <div class="pk-ctrl">
  ...
  function draw() {
    ...
    // fondo espacial con nebulosa
    const bg = ctx.createLinearGradient(0, 0, 0, AH); bg.addColorStop(0, '#0b0620'); bg.addColorStop(1, '#04060e'); ctx.fillStyle = bg; ctx.fillRect(0, 0, AW, AH);
    ctx.globalAlpha = 0.14; const neb = ctx.createRadialGradient(AW * 0.72, 54, 3, AW * 0.72, 54, 96); neb.addColorStop(0, '#6e2df5'); neb.addColorStop(1, 'transparent'); ctx.fillStyle = neb; ctx.fillRect(0, 0, AW, AH); ctx.globalAlpha = 1;
...
function openPerkyDance() {
  ...
  function draw() {
    ...
    // juicio con pop
    if (g.judge && g.t - g.judge.t < 0.5) { const k = 1 - (g.t - g.judge.t) / 0.5; const s = 1 + g.judgePop * 0.5; ctx.save(); ctx.globalAlpha = Math.min(1, k * 1.6); ctx.translate(AW / 2, 205); ctx.scale(s, s); ctx.fillStyle = g.judge.col; ctx.shadowColor = g.judge.col; ctx.shadowBlur = 10; ctx.font = 'bold 20px system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(g.judge.txt, 0, 0); ctx.restore(); }
...
function openPerkyHoops() {
  ...
  function resetBall() { g.ball = { x: AW / 2, y: AH - 22, vx: 0, vy: 0, rot: 0, flying: false, tb: false, tr: false }; g.aim = null; }
  function reset() { g = { score: 0, time: 45, combo: 0, over: false, hoopX: AW / 2, hoopY: 82, popup: null, parts: [], net: 0, stars: Array.from({ length: 26 }, () => ({ x: Math.random() * AW, y: Math.random() * (AH * 0.55), r: Math.random() * 1.2 + 0.3, tw: Math.random() * 7 })) }; resetBall(); wrap.querySelector('#pkScore').textContent = '0'; }
  reset();
  function hoopBurst(x, y, col) { for (let i = 0; i < 16; i++) { const a = Math.random() * 7, sp = 50 + Math.random() * 140; g.parts.push({ x, y, vx: Math.cos(a) * sp, vy: Math.sin(a) * sp - 30, life: 0.5 + Math.random() * 0.3, t: 0, col, r: 1.5 + Math.random() * 2.2 }); } }
  ...
  function update(dt) {
    ...
    if (g.net > 0) g.net = Math.max(0, g.net - dt * 3);
    for (const p of g.parts) { p.t += dt; p.vy += 240 * dt; p.x += p.vx * dt; p.y += p.vy * dt; }
    g.parts = g.parts.filter(p => p.t < p.life);
    const b = g.ball;
    if (b.flying) {
      const py = b.y;
      b.vy += GRAV * dt; b.x += b.vx * dt; b.y += b.vy * dt; b.rot += b.vx * dt * 0.06;
      ...
      if (!b.scored && py < g.hoopY && b.y >= g.hoopY && b.vy > 0 && Math.abs(b.x - g.hoopX) < RIMR - 3) {
        b.scored = true; g.combo++; g.net = 1;
        const swish = !b.tb && !b.tr;
        let pts = 2 + (g.combo - 1); if (swish) pts += 1;
        g.score += pts; g.popup = { txt: (swish ? '¡SWISH! +' : '+') + pts, col: swish ? '#2dc878' : '#ffd23e', t0: performance.now(), swish };
        hoopBurst(g.hoopX, g.hoopY + 8, swish ? '#2dc878' : '#ffd23e');
        wrap.querySelector('#pkScore').textContent = g.score; pkBlip(swish ? 820 : 680, 0.08); haptic(14);
  ...
  function draw() {
    ...
    ctx.clearRect(0, 0, AW, AH);
    // cielo nocturno
    const grd = ctx.createLinearGradient(0, 0, 0, AH); grd.addColorStop(0, '#221436'); grd.addColorStop(0.55, '#171129'); grd.addColorStop(1, '#0c0a18'); ctx.fillStyle = grd; ctx.fillRect(0, 0, AW, AH);
    // luna con halo
    ctx.save(); ctx.shadowColor = 'rgba(255,240,200,.6)'; ctx.shadowBlur = 22; ctx.fillStyle = '#f3e7c4'; ctx.beginPath(); ctx.arc(AW - 34, 34, 12, 0, 7); ctx.fill(); ctx.restore();
    // estrellas
    for (const s of g.stars) { ctx.globalAlpha = 0.4 + 0.5 * Math.abs(Math.sin(now / 700 + s.tw)); ctx.fillStyle = '#eaf2ff'; ctx.fillRect(s.x, s.y, s.r + 0.5, s.r + 0.5); } ctx.globalAlpha = 1;
    // skyline con ventanas iluminadas
    for (let i = 0; i < 6; i++) { const bx = 6 + i * 36, bw = 26, bh = 46 + (i % 3) * 26, byy = AH - bh; ctx.fillStyle = '#0e0b1c'; ctx.fillRect(bx, byy, bw, bh); ctx.fillStyle = 'rgba(255,210,120,.5)'; for (let wy = byy + 6; wy < AH - 6; wy += 10) for (let wx = bx + 4; wx < bx + bw - 4; wx += 8) if ((wx + wy + i) % 3 === 0) ctx.fillRect(wx, wy, 3, 4); }
    // piso/base sutil
    ctx.fillStyle = 'rgba(0,0,0,.35)'; ctx.fillRect(0, AH - 12, AW, 12);
    // canasta
    const hx = g.hoopX, hy = g.hoopY, boardX = hx + RIMR + 5;
    ctx.save(); ctx.shadowColor = 'rgba(255,255,255,.2)'; ctx.shadowBlur = 6;
    const bg2 = ctx.createLinearGradient(boardX, hy - 34, boardX + 6, hy + 6); bg2.addColorStop(0, '#f4f7ff'); bg2.addColorStop(1, '#c7d0e6'); ctx.fillStyle = bg2; ctx.fillRect(boardX, hy - 34, 6, 40); ctx.restore();
    ctx.strokeStyle = 'rgba(240,128,30,.5)'; ctx.lineWidth = 1; ctx.strokeRect(boardX - 1, hy - 20, 3, 14);
    // aro con brillo
    ctx.save(); ctx.shadowColor = '#f0801e'; ctx.shadowBlur = 12; ctx.strokeStyle = '#ff9838'; ctx.lineWidth = 3.5; ctx.beginPath(); ctx.moveTo(hx - RIMR, hy); ctx.lineTo(hx + RIMR, hy); ctx.stroke(); ctx.restore();
    // red (con oscilación al encestar)
    ctx.strokeStyle = 'rgba(255,255,255,.55)'; ctx.lineWidth = 1; const sway = g.net > 0 ? Math.sin(now / 40) * g.net * 3 : 0;
    for (let i = 0; i <= 6; i++) { const t = i / 6; ctx.beginPath(); ctx.moveTo(hx - RIMR + t * RIMR * 2, hy); ctx.lineTo(hx - RIMR * 0.55 + t * RIMR * 1.1 + sway * (t - 0.5) * 2, hy + 13 + g.net * 3); ctx.stroke(); }
    for (let r = 1; r <= 2; r++) { const yy = hy + r * 5; ctx.beginPath(); ctx.moveTo(hx - RIMR * (1 - r * 0.18), yy); ctx.lineTo(hx + RIMR * (1 - r * 0.18), yy); ctx.stroke(); }
    // trayectoria de puntería
    if (g.aim) {
      const dx = g.ball.x - g.aim.x, dy = g.ball.y - g.aim.y, k = 3.4;
      let sx = g.ball.x, sy = g.ball.y, vx = dx * k, vy = dy * k;
      for (let i = 0; i < 30; i++) { sx += vx * 0.02; sy += vy * 0.02; vy += GRAV * 0.02; if (i % 2 === 0) { ctx.globalAlpha = 0.7 * (1 - i / 30); ctx.fillStyle = '#eaf2ff'; ctx.beginPath(); ctx.arc(sx, sy, 2.2, 0, 7); ctx.fill(); } }
      ctx.globalAlpha = 1;
    }
    // partículas
    for (const p of g.parts) { ctx.globalAlpha = Math.max(0, 1 - p.t / p.life); ctx.fillStyle = p.col; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, 7); ctx.fill(); } ctx.globalAlpha = 1;
    // balón con sombreado y costuras que giran
    const b = g.ball;
    ctx.save(); ctx.translate(b.x, b.y); ctx.rotate(b.rot);
    const rg = ctx.createRadialGradient(-3, -3, 1, 0, 0, BR); rg.addColorStop(0, '#ffb46b'); rg.addColorStop(0.6, '#f0801e'); rg.addColorStop(1, '#c85f0e'); ctx.fillStyle = rg; ctx.beginPath(); ctx.arc(0, 0, BR, 0, 7); ctx.fill();
    ctx.strokeStyle = '#6f350c'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(-BR, 0); ctx.lineTo(BR, 0); ctx.moveTo(0, -BR); ctx.lineTo(0, BR); ctx.arc(0, 0, BR, -0.5, 0.5); ctx.arc(0, 0, BR, Math.PI - 0.5, Math.PI + 0.5); ctx.stroke();
    ctx.restore();
    // sombra del balón en el suelo
    if (!b.flying) { ctx.globalAlpha = 0.3; ctx.fillStyle = '#000'; ctx.beginPath(); ctx.ellipse(b.x, AH - 8, BR, 3, 0, 0, 7); ctx.fill(); ctx.globalAlpha = 1; }
    // HUD
    ctx.fillStyle = 'rgba(0,0,0,.4)'; ctx.fillRect(8, 8, AW - 16, 6);
    ctx.save(); ctx.shadowColor = g.time < 10 ? '#e0507a' : accent; ctx.shadowBlur = 8; ctx.fillStyle = g.time < 10 ? '#ff6a94' : accent; ctx.fillRect(8, 8, (AW - 16) * (g.time / 45), 6); ctx.restore();
    ctx.fillStyle = '#eaf2ff'; ctx.font = 'bold 11px system-ui'; ctx.textAlign = 'left'; ctx.fillText('⏱ ' + Math.ceil(g.time) + 's', 8, 30);
    if (g.combo > 1) { ctx.textAlign = 'right'; ctx.fillStyle = '#ffd23e'; ctx.font = 'bold 12px system-ui'; ctx.fillText('🔥 x' + g.combo, AW - 8, 30); }
    if (g.popup) { const k = (now - g.popup.t0) / 900; ctx.globalAlpha = Math.max(0, 1 - k); const s = 1 + Math.max(0, 0.4 - k) ; ctx.save(); ctx.translate(AW / 2, 150); ctx.scale(s, s); ctx.fillStyle = g.popup.col; ctx.shadowColor = g.popup.col; ctx.shadowBlur = 12; ctx.font = 'bold 20px system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(g.popup.txt, 0, 0); ctx.restore(); ctx.globalAlpha = 1; ctx.textBaseline = 'alphabetic'; }
  }
...
function openPerkyFish() {
  ...
  function splash(x, y, col, n) { for (let i = 0; i < n; i++) { const a = -Math.PI / 2 + (Math.random() - 0.5) * 1.6, sp = 40 + Math.random() * 120; g.splash.push({ x, y, vx: Math.cos(a) * sp, vy: Math.sin(a) * sp, life: 0.5 + Math.random() * 0.3, t: 0, col, r: 1 + Math.random() * 2 }); } }
  function action() {
    if (g.over) return;
    if (g.state === 'waiting') { g.state = 'cool'; g.cool = 0.7; g.msg = '¡Muy pronto! 🌀'; g.popup = { txt: '¡Paciencia!', col: '#f0a13e', t0: performance.now() }; haptic(10); }
      ...
      g.popup = { txt: f.n + ' ' + (pts >= 0 ? '+' : '') + pts, col: f.c, t0: performance.now(), big: true, rise: true };
      g.msg = pts >= 0 ? '¡Buena captura!' : 'Vaya… una bota';
      splash(AW / 2, 100, f.c, f.p > 40 ? 22 : 14);
  ...
  function update(dt) {
    ...
    g.time -= dt; if (g.time <= 0) { g.time = 0; g.over = true; showEnd(); return; }
    g.bob += dt;
    if (g.popup && performance.now() - g.popup.t0 > 1000) g.popup = null;
    for (const s of g.splash) { s.t += dt; s.vy += 300 * dt; s.x += s.vx * dt; s.y += s.vy * dt; }
    g.splash = g.splash.filter(s => s.t < s.life);
    // pez que ronda cuando falta poco para picar
    if (g.state === 'waiting' && g.wait < 1.1) g.fishX += g.fishDir * 40 * dt;
    if (g.state === 'waiting') { g.wait -= dt; if (g.wait <= 0) { g.state = 'bite'; g.bite = 0.9; g.msg = '¡PICA! ¡TOCA!'; splash(AW / 2, 100, '#bfe9ff', 8); pkBlip(880, 0.08); haptic(24); } }
    else if (g.state === 'bite') { g.bite -= dt; g.dip = Math.min(12, g.dip + dt * 48); if (g.bite <= 0) { g.state = 'cool'; g.cool = 0.7; g.msg = 'Se escapó 🌀'; g.popup = { txt: 'Se escapó', col: '#93a0bd', t0: performance.now() }; } }
    else if (g.state === 'cool') { g.cool -= dt; if (g.cool <= 0) cast(); }
  }
  function draw() {
    const now = performance.now(), WATER = 100;
    ctx.clearRect(0, 0, AW, AH);
    // cielo de atardecer
    const sky = ctx.createLinearGradient(0, 0, 0, WATER); sky.addColorStop(0, '#20344f'); sky.addColorStop(0.6, '#3a4d6e'); sky.addColorStop(1, '#c98a6a'); ctx.fillStyle = sky; ctx.fillRect(0, 0, AW, WATER);
    // luna
    ctx.save(); ctx.shadowColor = 'rgba(255,245,220,.7)'; ctx.shadowBlur = 20; ctx.fillStyle = '#fdf3d6'; ctx.beginPath(); ctx.arc(AW - 40, 30, 11, 0, 7); ctx.fill(); ctx.restore();
    // mar
    const sea = ctx.createLinearGradient(0, WATER, 0, AH); sea.addColorStop(0, '#1a94b0'); sea.addColorStop(0.5, '#0e6d89'); sea.addColorStop(1, '#073b52'); ctx.fillStyle = sea; ctx.fillRect(0, WATER, AW, AH - WATER);
    // reflejo de la luna en el agua
    ctx.globalAlpha = 0.18; ctx.fillStyle = '#fdf3d6'; for (let i = 0; i < 6; i++) { const yy = WATER + 6 + i * 8; ctx.fillRect(AW - 46 - Math.sin(g.bob * 2 + i) * 3, yy, 12 + i, 2); } ctx.globalAlpha = 1;
    // brillo/caustics + olas
    ctx.strokeStyle = 'rgba(255,255,255,.16)'; ctx.lineWidth = 1;
    for (let r = 0; r < 6; r++) { const yy = WATER + 16 + r * 30; ctx.beginPath(); for (let x = 0; x <= AW; x += 6) ctx.lineTo(x, yy + Math.sin((x + g.bob * 60 + r * 24) / 17) * 3); ctx.stroke(); }
    // línea de horizonte
    ctx.fillStyle = 'rgba(255,255,255,.25)'; ctx.fillRect(0, WATER - 1, AW, 1);
    // pez rondando bajo el agua
    if (g.state === 'waiting' && g.wait < 1.1) { ctx.globalAlpha = 0.5; ctx.fillStyle = '#04303f'; ctx.font = '16px system-ui'; ctx.textAlign = 'center'; ctx.fillText('🐟', AW / 2 + g.fishX, WATER + 40); ctx.globalAlpha = 1; }
    // caña + sedal
    const bx = AW / 2, by = WATER + (g.state === 'bite' ? g.dip : Math.sin(g.bob * 2) * 2);
    ctx.strokeStyle = 'rgba(255,255,255,.45)'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(bx, 0); ctx.lineTo(bx, by); ctx.stroke();
    // ondas alrededor del flotador
    for (let i = 0; i < 2; i++) { const rr = 6 + ((g.bob * 24 + i * 12) % 24); ctx.globalAlpha = Math.max(0, 0.3 - rr / 90); ctx.strokeStyle = '#dff3ff'; ctx.beginPath(); ctx.ellipse(bx, by + 3, rr, rr * 0.4, 0, 0, 7); ctx.stroke(); } ctx.globalAlpha = 1;
    // flotador con brillo
    ctx.save(); if (g.state === 'bite') { ctx.shadowColor = '#ff6a94'; ctx.shadowBlur = 12; } const rg = ctx.createRadialGradient(bx - 1.5, by - 1.5, 0.5, bx, by, 5.5); rg.addColorStop(0, '#ffffff'); rg.addColorStop(1, g.state === 'bite' ? '#e0507a' : '#cfe0ff'); ctx.fillStyle = rg; ctx.beginPath(); ctx.arc(bx, by, 5.5, 0, 7); ctx.fill(); ctx.restore();
    ctx.fillStyle = '#e0507a'; ctx.fillRect(bx - 5, by - 6, 10, 2.5);
    // salpicaduras
...
function plzPrerenderFloor() {
  ...
  const skyline = (a, b) => {
    const n = Math.abs(Math.round(b.x - a.x)); const dy = (b.y - a.y) / n;
    // estrellas
    c.fillStyle = 'rgba(180,205,255,.5)';
    for (let k = 0; k < n; k += 3) { const x = Math.round(a.x + Math.sign(b.x - a.x) * k), y = Math.round(a.y + dy * k); if ((k * 7) % 5 === 0) c.fillRect(x, y - WH + 4 + ((k * 3) % 10), 1, 1); }
    // edificios recortados en la base de la pared
    for (let k = 0; k < n; k += 4) {
      const x = Math.round(a.x + Math.sign(b.x - a.x) * k), y = Math.round(a.y + dy * k);
      const bh2 = 6 + ((k * 13) % 14);
      c.fillStyle = '#0a0e1d'; c.fillRect(x, y - bh2, 4, bh2);
      c.fillStyle = 'rgba(96,140,255,.4)'; if ((k) % 8 === 0) c.fillRect(x + 1, y - bh2 + 2, 1, 1);
    }
  };
  skyline(TL, TR); skyline(TL, LL);

  // guirnalda de bombillas de neón bajo el filo (puntos alternos azul/violeta/rosa)
  const garland = (a, b) => {
    const n = Math.abs(Math.round(b.x - a.x)); const dy = (b.y - a.y) / n;
    const cols = ['#27a9ff', '#6e2df5', '#e0507a', '#2dc878'];
    for (let k = 4; k < n; k += 7) {
      const x = Math.round(a.x + Math.sign(b.x - a.x) * k), y = Math.round(a.y + dy * k);
      c.fillStyle = '#0a0e1c'; c.fillRect(x, y - 1, 1, 3);       // hilo
      c.fillStyle = cols[(k / 7 | 0) % 4]; c.fillRect(x, y + 2, 2, 2);
    }
  };
...
// bola de discoteca pixel sobre la pista de baile
function plzDrawDiscoBall(c, now) {
  const p = plzIso(4.5, 2);          // centro de la pista
  const bx = Math.round(p.x), topY = -PLZ.WH - 2;    // colgada del techo
  const cy = topY + 22;
  // cable
  c.fillStyle = '#0a0e1c'; c.fillRect(bx, topY, 1, 22);
  // bola: cuadrícula de facetas que brillan por turnos
  const on = plaza.radio && plaza.radio.playing;
  for (let gx = -4; gx <= 4; gx += 2) for (let gy = -4; gy <= 4; gy += 2) {
    if (gx * gx + gy * gy > 20) continue;
    const lit = (Math.floor(now / (on ? 150 : 320)) + gx + gy) % 3 === 0;
    c.fillStyle = lit ? '#dbe8ff' : '#5b6fa6';
    c.fillRect(bx + gx, cy + gy, 2, 2);
  }
  c.fillStyle = '#0a0e1c'; c.fillRect(bx - 6, cy - 6, 1, 13); c.fillRect(bx + 6, cy - 6, 1, 13);   // contorno lateral
  // destellos que giran alrededor
  const spN = on ? 6 : 3;
  for (let s = 0; s < spN; s++) {
    const ang = now / 500 + s * (6.28 / spN);
    const sx = bx + Math.round(Math.cos(ang) * 12), sy = cy + Math.round(Math.sin(ang) * 6);
    c.globalAlpha = 0.4 + 0.4 * Math.abs(Math.sin(now / 200 + s));
    c.fillStyle = PLZ_DANCE_COLS[s % PLZ_DANCE_COLS.length] + '0.9)';
...
function plzDrawFurn(f, now) {
  ...
  if (!plzRasterMode && f.t !== 'portal' && f.t !== 'pool') plzDiamond(c, cx, base - Math.round(PLZ.TH / 2) + 2, PLZ.TW - 8, PLZ.TH - 4, 'rgba(0,0,0,.28)');
...
function plzDraw(now) {
  ...
  for (const [i, j] of plzR.dance) {
    ...
    const a = (0.12 + 0.20 * (phase * 0.5 + 0.5)) * boost;
    plzDiamond(ctx, p.x, p.y, PLZ.TW, PLZ.TH, col + a.toFixed(3) + ')');
  }
  ...
  // bola de discoteca colgando sobre la pista, con destellos y haz de luz
  plzDrawDiscoBall(ctx, now);
...
function plzDrawAvatar(e, now) {
  ...
  let cx = Math.round(p.x); const base = Math.round(p.y + PLZ.TH / 2);
  ...
  let danceLift = 0;
  if (dancing) {
    const beat = plaza.radio && plaza.radio.playing ? 300 : 360;
    cx += Math.round(Math.sin(now / (beat * 1.3)) * 3);
    danceLift = Math.abs(Math.sin(now / beat)) * 3;
  }
  const bob = walking ? Math.round(Math.abs(Math.sin(now / 100)) * 2)
            : dancing ? Math.round(danceLift)
  ...
  // reacción flotante (❤️🔥😂…): sube y se desvanece sobre la cabeza
  if (e.float) {
    const left = e.float.until - now;
    if (left <= 0) { e.float = null; }
    else {
      const t = 1 - left / 2600;
      c.globalAlpha = Math.min(1, left / 500);
      c.font = '13px serif'; c.textAlign = 'center'; c.textBaseline = 'alphabetic';
      c.fillText(e.float.emoji, cx + Math.round(Math.sin(t * 6) * 3), hy - 6 - Math.round(t * 22));
      c.globalAlpha = 1;
    }
  }
}
